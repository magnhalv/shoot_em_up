...
auto GLShaderProgram::initialize(const char* vertex_path, const char* fragment_path) -> bool {
  ...
  _vertex_source.last_modified = g_platform->get_file_last_modified(vertex_path);
  _fragment_source.last_modified = g_platform->get_file_last_modified(fragment_path);
  return true;
}

auto GLShaderProgram::relink_if_changed() -> void {
  auto vertex_last_modified = g_platform->get_file_last_modified(_vertex_source.path);
  auto fragment_last_modified = g_platform->get_file_last_modified(_fragment_source.path);
  if (vertex_last_modified > _vertex_source.last_modified || fragment_last_modified > _fragment_source.last_modified) {
