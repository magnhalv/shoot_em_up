...
struct Camera {
  ...
  void update_keyboard(UserInput& input) {
    vec2 dir = normalized(vec2(_direction.x, _direction.z));
  ...
  void update_cursor(f32 dx, f32 dy) {
    ...
    vec3 new_direction;
    new_direction.x = cos(radians(_yaw)) * cos(radians(_pitch));
    new_direction.y = sin(radians(_pitch));
    new_direction.z = sin(radians(_yaw)) * cos(radians(_pitch));
